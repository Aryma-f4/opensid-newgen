...
export default async function SettingAnalisisPage() {
  const rows = await prisma.setting_aplikasi.findMany({ where: { key: { contains: "analisis" } } })
  return (<div>
    <ContentHeader title="Setting Analisis" breadcrumb={[{ label: "Pengaturan" }, { label: "Analisis" }]} />
    <Box title="Pengaturan Analisis" noPadding>
