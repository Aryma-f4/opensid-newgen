...
import { createKelompok, updateKelompok, deleteKelompok } from "./actions"
...
export default function KelompokManager({
  data,
  masterRef,
  pendudukRef,
}: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      if (editing) await updateKelompok(editing, form)
      else await createKelompok(form)
  ...
  const handleDelete = async (ids: number[]) => {
    ...
    try { await deleteKelompok(ids); setPendingIds(new Set()) }
  ...
  return (
    <div>
      <ContentHeader title="Kelompok" subtitle="Daftar Kelompok" breadcrumb={[{ label: "Kelompok" }]} />

      <Box title={`Daftar Kelompok (${items.length})`} noPadding>
        ...
        <LteTable head={<><Th className="w-10"><input type="checkbox" checked={selectAll} onChange={() => { setSelectAll(!selectAll); setPendingIds(selectAll ? new Set() : new Set(items.map(p => p.id))) }} /></Th><Th>Kode</Th><Th>Nama Kelompok</Th><Th>Kategori</Th><Th>Ketua</Th><Th>Jml Anggota</Th><Th>No. SK Pendirian</Th><Th>Keterangan</Th><Th>Aksi</Th></>}>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 mb-12" onClick={(e) => e.stopPropagation()}>
            <form onSubmit={handleSubmit}>
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <h2 className="text-lg font-bold">{editing ? "Edit" : "Tambah"} Kelompok</h2>
