...
export default async function KelompokDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await prisma.kelompok.findUnique({
  ...
  const rows: [string, React.ReactNode][] = [
    ["Kode", item.kode],
    ["Nama", item.nama],
    ["Kategori", item.kelompok_master.kelompok],
    ["Tipe", item.tipe ?? "-"],
    ["Ketua", item.tweb_penduduk?.nama ? `${item.tweb_penduduk.nama} (${item.tweb_penduduk.nik})` : "-"],
  ...
  return (
    <div>
      <ContentHeader title="Detail Kelompok" breadcrumb={[{ label: "Kelompok", href: "/kelompok" }, { label: item.nama }]} />
      ...
      <div className="mt-4">
        <Box title={`Anggota Kelompok (${anggota.length})`} color="success" noPadding>
          <LteTable head={<><Th>NIK</Th><Th>Nama</Th><Th>Aksi</Th></>}>
            {anggota.length === 0 ? (
